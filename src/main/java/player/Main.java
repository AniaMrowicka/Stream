...
public class Main {
    public static void main(String[] args) {
        ...
        // Polecenia:
        System.out.println(" 1. Posortuj i wypisz automaty po ilości rozegranych meczy");
        posortowanePoIlosciMeczy(arcades);
        // 2. Posortuj i wypisz automaty po ilości zapisanych wyników (ilość score)
        System.out.println("2. Posortuj i wypisz automaty po ilości zapisanych wyników (ilość score)");
        posortowanePoIlosciPunktow(arcades);
        // 3. Znadjź i zwróć automat który ma najwięcej zapisanych wyników z gry Mario
        System.out.println("3. Znadjź i zwróć automat który ma najwięcej zapisanych wyników z gry Mario");
        System.out.println(najwiecejMario(arcades));
    ...
    private static Optional<Arcade> najwiecejMario(List<Arcade> arcades) {
        Optional<Arcade> mario1 = arcades.stream()
                .max(Comparator.comparing(
                        mario -> mario.getScoreList()
                                .stream()
                                .filter(count -> count.getGame().equals("MARIO"))
                                .map(count -> count.getGame()).count()
                ));
        return mario1;
    }
    ...
    private static void posortowanePoIlosciMeczy(List<Arcade> arcades) {
        arcades.stream()
                .sorted(Comparator.comparingInt(Arcade::getGamesPlayed))
                .forEach(System.out::println);
    }

    private static void automatyGdansk(List<Arcade> arcades) {
        arcades.stream()
                .filter(
                        auto -> auto.getCity().equalsIgnoreCase("Gdansk"))
                .forEach(System.out::println);
    }
}
